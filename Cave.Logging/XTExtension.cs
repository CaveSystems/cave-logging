...
namespace Cave
{
    ...
    public static class XTExtension
    {
        ...
        public static XT ToXT(this Exception ex, bool debug = false)
        {
            ...
            foreach (string str in ex.Message.SplitNewLine())
            {
            ...
            if (debug)
            {
                if (!string.IsNullOrEmpty(ex.Source))
                {
                    ...
                    result.Add(XTItem.NewLine);
                    foreach (string str in ex.Source.SplitNewLine())
                    {
                ...
                if (!string.IsNullOrEmpty(ex.StackTrace))
                {
                    ...
                    result.Add(XTItem.NewLine);
                    foreach (string str in ex.StackTrace.SplitNewLine())
                    {
                        if (str.Trim().Length == 0 || !ASCII.IsClean(str))
                        {
                            continue;
        ...
        public static void WriteHtml(this XT x, StringBuilder sb)
        {
            ...
            foreach (XTItem item in x.Items)
            {
                if (item.Color != color)
                {
                    // close old span
        ...
        /// <param name="x">The extended text.</param>
        /// <param name="sb">The StringBuilder.</param>
        public static void WriteHtml(this IXT x, StringBuilder sb)
        {
            x.ToXT().WriteHtml(sb);
        }
