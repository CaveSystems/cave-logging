...
class LogMessageFormatterTest
{
    ...
    public void Default()
    {
        var intl = new LogMessageFormatter() { FormatProvider = CultureInfo.InvariantCulture, UseArgumentColors = false };
        var de = new LogMessageFormatter() { FormatProvider = new CultureInfo("de-DE"), UseArgumentColors = false };
    ...
    public void DefaultColored()
    {
        ...
        var result1 = intl.FormatMessage(msg);
        Assert.AreEqual($"2023-02-23 23:02:23.200 Critical LogMessageFormatterTest> Critical test message number 2.5: This is even = True{Environment.NewLine}", result1.GetPlainText());
        Assert.AreEqual("<Inverse><Magenta>2023-02-23 23:02:23.200 Critical LogMessageFormatterTest><Reset> Critical test message number <Magenta>2.5<Reset>: This is even = <Green>True<Reset><Reset>\n", result1.Join());
        var result2 = de.FormatMessage(msg);
        Assert.AreEqual($"2023-02-23 23:02:23.200 Critical LogMessageFormatterTest> Critical test message number 2,5: This is even = True{Environment.NewLine}", result2.GetPlainText());
        Assert.AreEqual("<Inverse><Magenta>2023-02-23 23:02:23.200 Critical LogMessageFormatterTest><Reset> Critical test message number <Magenta>2,5<Reset>: This is even = <Green>True<Reset><Reset>\n", result2.Join());
    }
    ...
    public void ShortColored()
    {
        ...
        Assert.AreEqual("<Inverse><Magenta>C 23:02:23.200 LogMessageFormatterTest><Reset> Critical test message number <Magenta>2.5<Reset>: This is even = <Green>True<Reset><Reset>\n", result1.Join());
        var result2 = de.FormatMessage(msg);
